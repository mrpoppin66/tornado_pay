...
(function () {
  ...
  route(/^\/home$/, function (root) {
    ...
    root.innerHTML =
      ...
      '<div class="tile-grid">' +
      tile("🛍", "Услуги", "Создать заявку", "#/services") +
      tile("📋", "Мои заявки", "Статусы и чат", "#/orders") +
      tile("🧑‍💼", ME.role === "executor" ? "Кабинет исполнителя" : "Стать исполнителем", "", "#/executor") +
      tile("🤝", "Рефералы", "Приглашайте друзей", "#/referrals") +
      "</div>";
  });
  ...
  route(/^\/notifications$/, function (root) {
    ...
    return apiGet("/notifications").then(function (data) {
      ...
      if (!data.items.length) {
        root.innerHTML = '<h1>Уведомления</h1><div class="empty-state"><span class="emoji">🔕</span>Пока пусто</div>';
        return;
      }
      var html = '<h1>Уведомления</h1>';
      if (data.unread > 0) html += '<button class="secondary small" id="mark-read-btn">Отметить всё прочитанным</button>';
  ...
  route(/^\/services$/, function (root) {
    ...
    return apiGet("/services").then(function (services) {
      ...
      root.innerHTML = '<h1>Услуги</h1>' + services.map(function (s) {
        ...
        return '<div class="card tappable" data-id="' + s.id + '">' +
          '<div style="font-weight:600;font-size:15px">' + esc(s.name) + "</div>" +
          (s.description ? '<div class="muted" style="margin-top:4px">' + esc(s.description) + "</div>" : "") +
          '<div class="muted" style="margin-top:6px">От ' + fmt(minRub, 0) + " ₽</div>" +
          "</div>";
      }).join("");
    ...
    return apiGet("/me").then(function (me) {
      ...
      if (app && blockingStatuses.indexOf(app.status) !== -1) {
        ...
        if (app.status === "question") {
          html += '<textarea id="answer-input" placeholder="Ваш ответ"></textarea><button id="answer-btn">Отправить ответ</button>';
        }
        root.innerHTML = html;
  ...
  function renderExecutorDashboard(root) {
    ...
    return Promise.all([
      ...
      var html = '<h1>Кабинет исполнителя</h1>';
      ...
      html += '<div class="toggle-row"><span>Доступен для новых заявок</span><div class="switch' + (ME.executor_available ? " on" : "") + '" id="avail-switch"></div></div>';
      html += '<div class="toggle-row"><span>Уведомления о новых заявках</span><div class="switch' + (ME.executor_notify_enabled ? " on" : "") + '" id="notify-switch"></div></div>';
        ...
        if (!available.length) {
          ...
          html += available.map(function (o) {
            return '<div class="card"><div style="display:flex;justify-content:space-between">' +
              ...
              '<button class="small claim-btn" data-id="' + o.id + '">Взять в работу</button></div>';
          }).join("");
        }
      }
  ...
  route(/^\/referrals$/, function (root) {
    ...
    return apiGet("/referrals").then(function (r) {
      root.innerHTML =
        ...
        (r.link ? '<label>Ваша ссылка</label><input readonly id="ref-link" value="' + esc(r.link) + '"><button id="copy-ref-btn" class="secondary">📋 Скопировать ссылку</button>' : "") +
